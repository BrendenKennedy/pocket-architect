...
from pocket_architect.utils.rich_ui import display_info, display_error
...
def status_command(
    ...
    if not session:
        ...
        if provider_enum:
            console.print(f"Run 'pocket-architect cvat up --provider {provider_enum.value}' to start a session")
        else:
            console.print("Run 'pocket-architect cvat up' to start a session")
        return
