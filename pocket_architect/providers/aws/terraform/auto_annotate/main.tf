...
variable "subnet_id" {
  description = "Subnet ID for auto-annotate worker"
  type        = string
}
...
variable "instance_type" {
  description = "EC2 instance type (GPU instance for auto-annotation)"
  type        = string
  default     = "g4dn.xlarge"
}
...
# EC2 Instance
resource "aws_instance" "auto_annotate" {
  ami           = data.aws_ami.ubuntu.id
  instance_type = var.instance_type

  subnet_id              = var.subnet_id
  vpc_security_group_ids = [aws_security_group.auto_annotate.id]
  key_name               = var.ssh_key_name

  iam_instance_profile = aws_iam_instance_profile.auto_annotate.name
...
output "instance_id" {
  ...
  value       = aws_instance.auto_annotate.id
}

output "public_ip" {
  description = "Public IP address"
  value       = aws_instance.auto_annotate.public_ip
}
