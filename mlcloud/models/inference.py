...
from mlcloud.models.downloader import ModelDownloader
...
class InferenceEngine:
    ...
    def load_model(self, model_name: str) -> Any:
        ...
        if model.model_type.value == "sam2":
            from mlcloud.models.adapters.sam2 import SAM2Adapter
            adapter = SAM2Adapter(model)
        elif model.model_type.value.startswith("yolo11"):
            from mlcloud.models.adapters.yolo import YOLO11Adapter
            adapter = YOLO11Adapter(model)
        elif model.model_type.value == "grounding-dino":
            from mlcloud.models.adapters.grounding_dino import GroundingDINOAdapter
            adapter = GroundingDINOAdapter(model)
        elif model.model_type.value == "detectron2":
            from mlcloud.models.adapters.detectron2 import Detectron2Adapter
