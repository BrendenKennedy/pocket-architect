...
from mlcloud.core.blueprint import Blueprint, BlueprintSource
from mlcloud.providers import get_provider
from mlcloud.utils.rich_ui import display_success, display_error, display_info, display_cost_warning
...
def up(
    ...
    Examples:
        ...
        mlcloud cvat up --provider local --no-https
    """
    ...
    try:
        ...
        if provider_enum == Provider.AWS:
            ...
            if not subnet_id:
                from rich.prompt import Prompt
                from mlcloud.utils.validation import validate_subnet_id
                try:
            ...
            if not my_ip_cidr:
                from mlcloud.utils.validation import get_my_ip, validate_cidr
                try:
                    my_ip = get_my_ip()
                    if my_ip:
...
def sync(
    ...
    Examples:
        mlcloud cvat sync --provider aws
        mlcloud cvat sync --direction down --path ./annotations
    """
    provider_enum = _get_provider_from_string(provider)
    ...
    if not session:
        ...
        console.print("Run 'mlcloud cvat up' first to deploy CVAT")
        raise typer.Exit(1)
